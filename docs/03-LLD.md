...
1. 邮箱对应账号存在且状态为active；
2. 所选角色等于`users.role`；
3. 密码哈希校验通过；
4. 由服务端账号数据签发带角色的访问令牌。

任一条件不成立统一返回`AUTH_004`，避免泄露账号是否存在或真实角色。登录成功后前端不允许修改角色。
...
- 未登录显示登录/家长注册页；
- 登录时选择角色，`LoginRequest.role`必须与账号真实角色一致；
- 登录后显示真实身份，不提供角色切换按钮；
- 家长、学生、管理员使用独立导航配置；
- API错误映射为中文行动提示；
...
- `GET /dashboard`按当前令牌角色实时聚合数据库，不接受前端传入角色；
- 已完成练习只统计`PracticeSession.status=completed`；
- 待复习错题排除`state=mastered`；
- 待确认资料只统计`awaiting_confirmation`；
- 管理员题目数量只统计题目active且当前版本approved/published；
- 响应包含`generated_at`，用于显示统计更新时间；
- 练习科目通过`GET /questions/subjects?grade=`读取当前年级已审核发布题目；
- 题量最大值由对应科目的真实题目数量决定，界面上限为10题；
- 前端不提供固定统计值、固定科目或固定题量；
- 业务数据为空时显示空状态，不使用演示数据填充。
