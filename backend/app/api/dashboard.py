...
def dashboard(
    ...
    if user.role == "admin":
        student_count = (
            db.scalar(select(func.count(Student.id)).where(Student.deleted_at.is_(None))) or 0
        )
        completed_practice_count = (
            db.scalar(
        ...
        actions = [
            _action("查看已发布题库", "/questions", True),
        ]
        notices = []
        if active_question_count == 0:
            ...
            notices.append(
                {
                    "level": "fact",
                    "text": f"当前共有 {active_question_count} 道已发布题目。",
                }
            )
        if pending_documents:
            notices.append(
                {
                    "level": "fact",
                    "text": f"当前有 {pending_documents} 份学习资料等待家长确认。",
                }
            )
    else:
        ...
        students = list(
            db.scalars(
                select(Student).where(*student_conditions).order_by(Student.created_at)
            ).all()
        )
        ...
        if user.role == "parent":
            ...
            actions = [
                ...
                _action(
                    ...
                    "/documents",
                    bool(students),
                    "请先创建学生档案" if not students else None,
                ),
            ]
        else:
            metrics = [
            ...
            actions = [
                ...
                _action(
                    ...
                    "/documents",
                    bool(students),
                    "账号尚未绑定学生档案" if not students else None,
                ),
            ]
        ...
        if not students:
            notices.append(
                {"level": "insufficient_data", "text": "当前账号尚未关联学生档案。"}
            )
        if completed_practice_count == 0:
            notices.append(
                {
                    "level": "insufficient_data",
                    "text": "暂无已完成练习，当前不能形成学习趋势判断。",
                }
            )
        else:
            notices.append(
                {
                    "level": "fact",
                    "text": f"已完成 {completed_practice_count} 次练习。",
                }
            )
        if wrong_count:
            notices.append(
                {"level": "fact", "text": f"有 {wrong_count} 道错题等待复习。"}
            )
        if pending_documents:
            notices.append(
                {
                    "level": "fact",
                    "text": f"有 {pending_documents} 份资料等待家长确认。",
                }
            )
